    ...
    <div className="flex min-h-full flex-1 flex-col">
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col items-center px-6 py-24 text-center">
        ...
        <h1 className="mt-4 max-w-2xl font-display text-5xl leading-[1.1] text-text-primary sm:text-6xl">
          Know your site&rsquo;s <em className="italic">real</em> health
        </h1>
        ...
        <div className="mt-16 flex flex-wrap items-center justify-center gap-2">
          {CHECKS.map((c) => (
            <span
              key={c.label}
              title={c.detail}
              className="inline-flex items-center gap-1.5 rounded-full border border-border px-[11px] py-1 font-mono-label text-[11px] uppercase tracking-wide text-text-secondary"
            >
              {c.label}
            </span>
          ))}
